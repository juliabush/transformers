    ...
    for model_class in self.all_model_classes:
        ...
        for batch_size in [7]:
                ...
                if value.shape[0] < input_data_batch_size:
                    ...
                    if torch.is_floating_point(value):
                        ...
                        extension = torch.randint(
                            high=5, size=size, dtype=value.dtype, device=torch_device
                        )
            ...
            if not use_attention_mask:
                ...
                if dummy_attention_mask.shape[0] < batch_size:
                    ...
                    extension = torch.ones(
                        size=size, dtype=dummy_attention_mask.dtype, device=torch_device
                    )
            ...
            if "logits_per_text" in outputs_eager:
                ...
                "language_model_outputs" in outputs_eager
                and "blip" in model_class.__name__.lower()
            ):
                outputs_eager = outputs_eager["language_model_outputs"]
                outputs_sdpa = outputs_sdpa["language_model_outputs"]
                key = (
                    "hidden_states"
                    if "hidden_states" in outputs_eager
            ...
            outputs_magnitude = float(
                (torch.max(logits_sdpa.abs().amax(), logits_eager.abs().amax()))
                .detach()
                .to("cpu")
            )
            ...
            if np.mean(results) < 0.8:
                mean_relative_diff = (
                    (logits_sdpa - logits_eager).abs() / (logits_eager.abs() + 1e-12)
                ).mean()
                raise ValueError(
...
def _test_eager_matches_batched_and_grouped_inference(self, name, dtype):
    if not self.all_model_classes[0]._can_set_experts_implementation():
        self.skipTest(
            f"{self.all_model_classes[0].__name__} does not support grouped_mm"
        )
    ...
    for model_class in self.all_model_classes:
        ...
        with torch.no_grad():
            ...
            with (
                ...
                patch.dict(
                    ...
                    {
                        "batched_mm": mock_batched_mm_forward,
                        "grouped_mm": mock_grouped_mm_forward,
                    },
                ),
            ):
        ...
        torch.testing.assert_close(
            outputs_eager, outputs_batched_mm, rtol=1e-4, atol=1e-4
        )
        torch.testing.assert_close(
            outputs_eager, outputs_grouped_mm, rtol=1e-4, atol=1e-4
        )
...
class ModelTesterMixin:
    ...
    def test_num_layers_is_small(self):
        ...
        target_num_hidden_layers = exceptional_num_hidden_layers.get(
            type(self).__name__, 2
        )

        if hasattr(self.model_tester, "num_hidden_layers") and isinstance(
            self.model_tester.num_hidden_layers, int
        ):
        ...
        if (
            ...
            if isinstance(self.model_tester.vision_config, dict):
                assert (
                    self.model_tester.vision_config["num_hidden_layers"]
        ...
        if (
            ...
            if isinstance(self.model_tester.text_config, dict):
                assert (
                    self.model_tester.text_config["num_hidden_layers"]
                    <= target_num_hidden_layers
                )
            else:
                assert (
            ...
            if isinstance(first, tuple) and isinstance(second, tuple):
                for tensor1, tensor2 in zip(first, second):
                    torch.testing.assert_close(
                        tensor1,
                        tensor2,
                        msg="Running save/load and forward yields different results",
                    )
            else:
                torch.testing.assert_close(
                    first,
    ...
    def test_keep_in_fp32_modules_exist(self):
        ...
        for model_class in self.all_model_classes:
            with self.subTest(model_class.__name__):
                ...
                if (
                    len(model._keep_in_fp32_modules) == 0
                    and len(model._keep_in_fp32_modules_strict) == 0
                ):
                    self.skipTest(
            ...
            with self.subTest(model_class.__name__):
                ...
                with tempfile.TemporaryDirectory() as tmpdirname:
                    ...
                    self.assertFalse(
                        ...
                        & model._keep_in_fp32_modules_strict,
                    ...
                    for name, param in model.state_dict().items():
                        if any(
                            re.search(rf"(?:^|\.){k}(?:\.|$)", name)
                            for k in all_fp32_modules
                        ):
                            self.assertTrue(
                                param.dtype == torch.float32,
                                f"{name} not upcasted to fp32",
                            )
                        else:
                            self.assertTrue(
                                param.dtype == torch.float16,
                                f"{name} was upcasted but it should NOT be",
                            )

                    # When reloading in bf16, only keep_in_fp32_modules_strict should be upcasted
                    model = model_class.from_pretrained(
                        tmpdirname, dtype=torch.bfloat16
                    )
                    for name, param in model.state_dict().items():
                        if any(
                            re.search(rf"(?:^|\.){k}(?:\.|$)", name)
                            for k in model._keep_in_fp32_modules_strict
                        ):
                            self.assertTrue(
                                param.dtype == torch.float32,
                                f"{name} not upcasted to fp32",
    ...
    def test_load_contiguous_weights(self):
        ...
        for model_class in self.all_model_classes:
            ...
            model = model_class(config)
            self.assertTrue(
                all(param.is_contiguous() for param in list(model.parameters()))
            )
    ...
    def test_enable_input_require_grads_with_gradient_checkpointing(self):
        ...
        for model_class in self.all_model_classes:
            ...
            if (
                loss_tensor is None
                or not isinstance(loss_tensor, torch.Tensor)
                or not loss_tensor.requires_grad
            ):
    ...
    def test_can_init_all_missing_weights(self):
        ...
        if match_object := re.search(
            r"^# Copyright (\d{4})", source_code, re.MULTILINE | re.IGNORECASE
        ):
        ...
        for model_class in self.all_model_classes:
            ...
            with seeded_weight_init():
                ...
                # by _init_weights()
                model_from_pretrained = model_class.from_pretrained(
                    None, config=copy.deepcopy(config), state_dict={}
                )
            ...
            for weight in different_weights.copy():
                weight_name, immediate_parent_class, pretrained_parent_class = (
                    find_parent_traceback(weight, model_from_init)
                )

                # We cannot control timm model weights initialization, so skip in this case
                if (
                    pretrained_parent_class == "TimmWrapperPreTrainedModel"
                    and "timm_model." in weight
                ) or (
                ...
                traceback = f"`{weight_name}` in module `{immediate_parent_class}` called from `{pretrained_parent_class}`\n"
    ...
    def test_init_weights_can_init_buffers(self):
        ...
        for model_class in self.all_model_classes:
            ...
            for buffer in different_buffers.copy():
                buf_name, immediate_parent_class, pretrained_parent_class = (
                    find_parent_traceback(buffer, model_from_init)
                )
                # Add it to the traceback
        ...
        for model_class in self.all_model_classes:
            ...
            try:
                ...
                if "Cannot copy out of meta tensor; no data!" in str(
                    e
                ) or "Tensor on device meta is not on the expected device cpu!" in str(
                    e
                ):
                    raise ValueError(
    ...
    def check_training_gradient_checkpointing(self, gradient_checkpointing_kwargs=None):
        ...
        for model_class in self.all_model_classes:
            with self.subTest(model_class.__name__):
                ...
                loss.backward()
                grad_expected_params = [
                    (n, p) for n, p in model.named_parameters() if p.grad is not None
                ]
                non_zero_grads_normal = {
                    n for n, p in grad_expected_params if p.grad.abs().sum() > 0
                }
    ...
    def _set_subconfig_attributes(self, config, attribute_name, value):
        ...
        for k in config.sub_configs:
            ...
            if getattr(config, k) is not None:
                ...
                self._set_subconfig_attributes(
                    getattr(config, k), attribute_name, value
                )
    ...
    def test_retain_grad_hidden_states_attentions(self):
        config, inputs_dict = (
            self._prepare_config_and_inputs_for_retain_grad_hidden_states_attentions()
        )
        self._set_subconfig_attributes(config, "output_hidden_states", True)
    ...
    def test_resize_embeddings_untied(self):
        ...
        try:
            ...
            print(
                f"Could not set text config's `tie_word_embeddings` for model type `{model_type}`: {e}"
            )
        inputs_dict.pop("labels", None)
            ...
            with tempfile.TemporaryDirectory() as d:
                ...
                with self.subTest(model_class):
                    ...
                    for shared_names in shared_ptrs.values():
                        reloaded_ptrs = {
                            reloaded_state[k].data_ptr() for k in shared_names
                        }
                        self.assertEqual(
                            len(reloaded_ptrs),
                            1,
    ...
    def test_tied_weights_keys(self):
        ...
        for model_class in self.all_model_classes:
            ...
            if len(tied_weight_keys) == 0 and not getattr(
                original_config, "tie_word_embeddings", None
            ):
                continue
            ...
            with self.subTest(msg=f"Testing {model_class}"):
                with tempfile.TemporaryDirectory() as tmp_dir:
                    ...
                    if len(top_linear_modules) == 0:
                        ...
                        if any(
                            module.__class__.__name__.endswith("ClassificationHead")
                            for module in new_model.children()
                        ):
                            head_name, head_module = next(
                                (name, module)
                                for name, module in new_model.named_children()
                                if module.__class__.__name__.endswith(
                                    "ClassificationHead"
                                )
                            )
                        # nn.Sequential case
                        ...
                        # Unknown at this point -> skip (only xlm, perceiver, levit, flaubert, audio_spectrogram_transformer as of 23/09/2025)
                            ...
                            self.skipTest(
                                "Could not locate the classification Linear layer."
                            )
                        top_linear_modules = [
                    ...
                    mismatched_modules = [
                        name
                        for name, module in top_linear_modules
                        if module.out_features == 42
                    ]
                    ...
                    for k1 in new.keys():
                        ...
                        if not any(
                            k1.startswith(mismatched_module)
                            for mismatched_module in mismatched_modules
                        ):
                            torch.testing.assert_close(
                                v1, v2, msg=f"{k1} and  {k2} do not match: {v1} != {v2}"
                            )
                        # Check that the dims are indeed mismatched between old and new models
                            ...
                            self.assertLessEqual(
                                v1.data.mean().item(), 1e-1, f"Issue with {k1}"
                            )
    ...
    def flash_attn_inference_equivalence(
        ...
        for model_class in self.all_model_classes:
            ...
            if (
                not model_class._supports_attention_backend
                and not attn_implementation.startswith("flash_attention")
            ):
                continue
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                first_inputs = {
                    model.main_input_name: main_input,
                    "output_hidden_states": True,
                }
                ...
                if model.config.is_encoder_decoder:
                    decoder_input_ids = inputs_dict.get(
                        "decoder_input_ids", first_inputs.get("input_ids")
                    )
                ...
                # Check the results
                print("max diff 1:", (logits_1_eager - logits_1_fa).abs().max())
                print(
                    "dtype eager:", logits_1_eager.dtype, "dtype fa:", logits_1_fa.dtype
                )
                torch.testing.assert_close(
                    logits_1_eager, logits_1_fa, atol=atol, rtol=rtol
                )
                if padding_side == "left":
                    print("max diff 2:", (logits_2_eager - logits_2_fa).abs().max())
                    torch.testing.assert_close(
                        logits_2_eager[1:], logits_2_fa[1:], atol=atol, rtol=rtol
                    )
                else:
                    torch.testing.assert_close(
                        logits_2_eager[:-1], logits_2_fa[:-1], atol=atol, rtol=rtol
    ...
    def test_flash_attn_kernels_inference_equivalence(self):
        self.flash_attn_inference_equivalence(
            attn_implementation="kernels-community/flash-attn3", padding_side="left"
        )
    ...
    def test_flash_attn_2_inference_equivalence(self):
        self.flash_attn_inference_equivalence(
            attn_implementation="flash_attention_2", padding_side="left"
        )
    ...
    def test_flash_attn_2_inference_equivalence_right_padding(self):
        self.flash_attn_inference_equivalence(
            attn_implementation="flash_attention_2", padding_side="right"
        )
    ...
    def test_flash_attn_3_inference_equivalence(self):
        self.flash_attn_inference_equivalence(
            attn_implementation="flash_attention_3", padding_side="left"
        )
    ...
    def test_flash_attn_3_inference_equivalence_right_padding(self):
        self.flash_attn_inference_equivalence(
            attn_implementation="flash_attention_3", padding_side="right"
        )
    ...
    def test_flash_attn_4_inference_equivalence(self):
        self.flash_attn_inference_equivalence(
            attn_implementation="flash_attention_4", padding_side="left"
        )
    ...
    def test_flash_attn_4_inference_equivalence_right_padding(self):
        self.flash_attn_inference_equivalence(
            attn_implementation="flash_attention_4", padding_side="right"
        )
    ...
    def test_attn_implementation_composite_models(self):
        ...
        for model_class in self.all_model_classes:
            ...
            model = model_class(config)
            self.assertTrue(
                model.config.get_text_config(decoder=True)._attn_implementation
                == "eager"
            )
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                model.save_pretrained(tmpdirname)
                new_model = model.from_pretrained(
                    tmpdirname, attn_implementation=attn_implementation_per_subconfig
                )
    ...
    def test_sdpa_can_dispatch_composite_models(self):
        ...
        for model_class in self.all_model_classes:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                vision_model_name = (
                    vision_model_name[0] if len(vision_model_name) > 0 else None
                )
                language_model_name = [
                    name for name in language_model_names if hasattr(model_sdpa, name)
                ]
                language_model_name = (
                    language_model_name[0] if len(language_model_name) > 0 else None
                )
    ...
    def test_sdpa_can_dispatch_on_flash(self):
        ...
        if device_type == "cuda" and major < 8:
            ...
            self.skipTest(
                reason="This test requires a Nvidia or AMD GPU, or an Intel XPU"
            )
        ...
        for model_class in self.all_model_classes:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                model.save_pretrained(tmpdirname)
                model = model_class.from_pretrained(
                    tmpdirname, dtype=torch.float16, attn_implementation="sdpa"
                )
                model.to(torch_device)
    ...
    def test_sdpa_can_compile_dynamic(self):
        ...
        for model_class in self.all_model_classes:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                model.save_pretrained(tmpdirname)
                model = model_class.from_pretrained(
                    tmpdirname, dtype=torch.bfloat16, attn_implementation="sdpa"
                )
                model.to(torch_device)
    ...
    def flash_attn_can_dispatch_composite_models(self, attn_implementation: str):
        ...
        if not is_torch_bf16_available_on_device(torch_device):
            self.skipTest(
                f"bfloat16 not supported on {torch_device} (on the specific device currently used)"
            )
        ...
        expected_attn_implementations = _expected_attn_implementations(
            attn_implementation
        )
                ...
                if not supports_fa_all_modules:
                    ...
                    for key in model_fa.config:
                        if isinstance(getattr(model_fa.config, key), PreTrainedConfig):
                            ...
                            self.assertIn(
                                sub_config._attn_implementation,
                                expected_attn_implementations,
                            )
                    ...
                    for name, submodule in model_fa.named_modules():
                        ...
                        if (
                            ...
                            in expected_attn_implementations
                        ):
                            has_fa = True
                            break
                    if not has_fa:
                        raise ValueError(
                            f"The {attn_implementation} model should have {attn_implementation} layers"
                        )
    ...
    def test_flash_attn_2_can_dispatch_composite_models(self):
        self.flash_attn_can_dispatch_composite_models(
    ...
    def test_flash_attn_2_fp32_ln(self):
        ...
        for (
            ...
            self.all_generative_model_classes
        ):  # TODO: this test should run on all classes instead
            ...
            if not all(
                ...
                self.skipTest(
                    reason="At least some parts of this model do not support flash attention"
                )
    ...
    @pytest.mark.torch_compile_test
    @slow
    def test_flash_attn_2_can_compile_with_attention_mask_None_without_graph_break(
        self,
    ):
    ...
    def flash_attn_from_config(
        self, attn_implementation: str, test_fwd_in_train: bool = True
    ):
        r"""
        ...
        for (
            ...
            if not model_class._supports_flash_attn:
                self.skipTest(
                    f"{model_class.__name__} does not support {attn_implementation}"
                )
            ...
            model = model_class(
                config
            )  # let's construct it here to see if any submodels can't support flash attn
            if not all(
                submodel._supports_flash_attn
                for submodel in model.modules()
                if isinstance(submodel, PreTrainedModel)
                ...
                self.skipTest(
                    reason=f"At least some parts of this model do not support {attn_implementation}"
                )
    ...
    def test_sliding_window_mask(self):
        ...
        if not (
            hasattr(config, "sliding_window") and hasattr(config, "use_sliding_window")
        ):
        ...
        inputs["attention_mask"] = (
            torch.ones(batch_size, seq_len).to(torch.int64).to(torch_device)
        )
        for model_class in self.all_model_classes:
            ...
            model = model_class._from_config(
                new_config, attn_implementation="eager"
            ).to(torch_device)
            model.eval()
            layer_types = getattr(
                model.config,
            ...
            for layer_attention, layer_type in zip(attentions, layer_types):
                if layer_type == "sliding_attention":
                    self.assertTrue(
                        (layer_attention[:, :, ~sliding_mask] == 0).all().item()
                    )
                else:
                    self.assertFalse(
                        (layer_attention[:, :, ~sliding_mask] == 0).all().item()
    ...
    def test_torch_compile_for_training(self):
        ...
        attn_implementation = getattr(
            self, "_torch_compile_train_attn_implementation", None
        )
    ...
    def test_torch_export(self, atol=1e-4, rtol=1e-4):
        ...
        with open(inspect.getfile(self.all_model_classes[0]), "r") as f:
            ...
            if "for q, k, v in zip(*splits)" in source_code:
                self.skipTest(
                    reason="Model architecture uses chunked attention which is not torch exportable"
                )
            # Skip MoEs that don't support batched_mm experts implementation
            if (
                "for expert" in source_code
                and "use_experts_implementation" not in source_code
            ):
                self.skipTest(
                    reason="Model architecture uses eager MoE implementation which is not torch exportable"
                )
            # Skip models that use get_rope_index which is not torch exportable
            if "get_rope_index" in source_code:
                self.skipTest(
                    reason="Model architecture uses get_rope_index which is not torch exportable"
                )
        ...
        def _prepare_for_export(model, inputs_dict):
            ...
            if (
                model._can_set_attn_implementation()
                and model.config.model_type != "videomae"
            ):
                try:
            ...
            with self.subTest(model_class.__name__):
                ...
                try:
                    exported_program = torch.export.export(
                        model, args=(), kwargs=copy.deepcopy(inputs_dict)
                    )
                except Exception as e:
                    raise e
                ...
                torch.testing.assert_close(
                    exported_outputs, eager_outputs, atol=atol, rtol=rtol
                )
    ...
    def _prepare_config_headdim(config, requested_dim):
        ...
        def update_config_headdim(config, requested_dim):
            ...
            if (
                ...
                num_attn_heads = (
                    num_attn_heads
                    if isinstance(num_attn_heads, int)
                    else max(num_attn_heads)
                )
                head_dim = (
                    head_dim
                    if head_dim is not None
                    else config.hidden_size // num_attn_heads
                )
            ...
            if (
                ...
                decoder_head_dim = (
                    config.decoder_hidden_size // config.decoder_num_attention_heads
                )
            ...
            if (
                ...
                scaling_factor = max(
                    requested_dim // (sum(config.rope_parameters["mrope_section"]) * 2),
                    1,
                )
                config.rope_parameters["mrope_section"] = [
                    section * scaling_factor
                    for section in config.rope_parameters["mrope_section"]
                ]
    ...
    def test_flex_attention_with_grads(self):
        for model_class in self.all_model_classes:
            ...
            if not all(
                ...
                self.skipTest(
                    reason="At least some parts of this model do not support flex attention"
                )
            ...
            dummy_inputs = {
                model.main_input_name: inputs_dict[model.main_input_name].to(
                    torch_device
                )
            }
            for key in getattr(self, "additional_model_inputs", []):
            ...
            if config.is_encoder_decoder:
                dummy_inputs["decoder_input_ids"] = inputs_dict["decoder_input_ids"].to(
                    torch_device
                )
                dummy_inputs["decoder_attention_mask"] = inputs_dict[
    ...
    def test_can_load_with_device_context_manager(self):
        ...
        for model_class in self.all_model_classes:
            ...
            self.assertEqual(
                unique_devices,
                {device},
                f"All parameters should be on {device}, but found {unique_devices}.",
            )
    ...
    def test_config_attn_implementation_setter(self):
        ...
        def check_attn_implementation_setter(
            config: PreTrainedConfig, attn_implementation: str
        ):
            ...
            for attribute_value in config.__dict__.values():
                if isinstance(attribute_value, PreTrainedConfig):
                    check_attn_implementation_setter(
                        attribute_value, attn_implementation
                    )
    ...
    def test_internal_model_config_and_subconfig_are_same(self):
        ...
        for model_class in self.all_model_classes:
            ...
            for submodule in model.modules():
                # This is a submodel
                if (
                    isinstance(submodule, PreTrainedModel)
                    ...
                    for subconfig_key in subconfig_keys:
                        ...
                        subconfig_from_model_config = getattr(
                            model.config, subconfig_key
                        )
                        if (
                            subconfig_from_model_config is not None
                            and subconfig_from_model_config.__class__
                            == subconfig_from_model_internal.__class__
                    ...
                    if len(matching_sub_configs) > 0:
                        self.assertTrue(
                            any(
                                subconfig_from_model_config
                                is subconfig_from_model_internal
    ...
    def test_can_set_attention_dynamically(self):
        ...
        for model_class in self.all_model_classes:
            if not model_class._can_set_attn_implementation():
                self.skipTest(
                    reason="This model does not support setting its attention dynamically"
                )
            ...
            for subconfig_key in model.config.sub_configs:
                if getattr(config, subconfig_key) is not None:
                    self.assertTrue(
                        getattr(model.config, subconfig_key)._attn_implementation
                        == "eager"
                    )
            ...
            with self.assertRaisesRegex(
                ValueError, 'Specified `attn_implementation="foo"` is not supported'
            ):
        ...
        for model_class in self.all_model_classes:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                for dtype in [
                    ...
                    model_torch_dtype = model_class.from_pretrained(
                        tmpdirname, torch_dtype=dtype
                    )
                    model_dtype = model_class.from_pretrained(tmpdirname, dtype=dtype)

                    for (k1, v1), (k2, v2) in zip(
                        model_torch_dtype.named_parameters(),
                        model_dtype.named_parameters(),
                    ):
    ...
    def test_tp_plan_matches_params(self):
        ...
        if config.base_model_tp_plan is None and all(
            getattr(getattr(config, key), "base_model_tp_plan", None) is None
            for key in config.sub_configs
        ):
        ...
        config_to_set.first_k_dense_replace = (
            1  # means that the first layer (idx 0) will be MLP, then MoE
        )
        ...
        for model_class in self.all_model_classes:
            ...
            for pattern in tp_plan:
                ...
                pattern_usage[pattern] = any(
                    _get_parameter_tp_plan(param, {pattern: ""}, is_weight=True)
                    is not None
                    for param in param_names
                ) or any(
                    _get_parameter_tp_plan(module, {pattern: ""}, is_weight=False)
                    is not None
            ...
            self.assertTrue(
                len(unused_entries) == 0,
                f"The following entries of the TP-plan are not valid: {unused_entries}",
            )
            ...
            with self.subTest(model_class.__name__):
                ...
                with tempfile.TemporaryDirectory() as tmpdirname:
                    ...
                    state_dict = load_file(
                        os.path.join(tmpdirname, "model.safetensors")
                    )
                ...
                if check_keys_were_modified:
                    ...
                    self.assertNotEqual(
                        sorted(serialized_keys),
                        sorted(model_keys),
                        "No key mapping was performed!",
                    )
                ...
                for conversion in conversions:
                    for source_pattern in conversion.source_patterns:
                        ...
                        if isinstance(conversion, WeightRenaming):
                            ...
                            if captured_group:
                                target_pattern_reversed = (
                                    target_pattern_reversed.replace(
                                        r"\1", captured_group
                                    )
                                )
                            if any(
                                re.search(target_pattern_reversed, k)
                                for k in model.all_tied_weights_keys.keys()
                            ):
                                continue
                        num_matches = sum(
                            re.search(source_pattern, key) is not None
                ...
                with tempfile.TemporaryDirectory() as tmpdirname:
                    ...
                    model.save_pretrained(tmpdirname)
                    state_dict_saved_from_init = load_file(
                        os.path.join(tmpdirname, "model.safetensors")
                    )
                    ...
                    self.assertTrue(
                        compare_state_dicts(
                            state_dict_saved_from_init, state_dict_saved_from_pretrained
                        )
                    )
        ...
        for model_class in self.all_model_classes:
            ...
            with self.subTest(model_class.__name__):
                ...
                with tempfile.TemporaryDirectory() as tmpdirname:
                    ...
                    self.assertTrue(
                        compare_state_dicts(
                            model.state_dict(), model_reloaded.state_dict()
                        )
                    )
    ...
    def _text_features_prepare_config_and_inputs(self):
        ...
        if hasattr(self.model_tester, "text_model_tester"):
            _, inputs_dict = (
                self.model_tester.text_model_tester.prepare_config_and_inputs_for_common()
            )
        else:
            inputs_dict = {
                key: value
    ...
    def _image_features_prepare_config_and_inputs(self):
        ...
        if hasattr(self.model_tester, "vision_model_tester"):
            _, inputs_dict = (
                self.model_tester.vision_model_tester.prepare_config_and_inputs_for_common()
            )
        else:
            inputs_dict = {
                key: value
    ...
    def _audio_features_prepare_config_and_inputs(self):
        ...
        if hasattr(self.model_tester, "audio_model_tester"):
            _, inputs_dict = (
                self.model_tester.audio_model_tester.prepare_config_and_inputs_for_common()
            )
        else:
            inputs_dict = {
                key: value
    ...
    def _video_features_prepare_config_and_inputs(self):
        ...
        if hasattr(self.model_tester, "video_model_tester"):
            _, inputs_dict = (
                self.model_tester.video_model_tester.prepare_config_and_inputs_for_common()
            )
        else:
            key_mappings = {
            ...
            inputs_dict = {
                key: value
                for key, value in inputs_dict.items()
                if "video" in key or key in allowed_non_video_keys
            }
        return config, inputs_dict
    ...
    def _text_features_get_expected_num_attentions(self, model_tester=None):
        ...
        if hasattr(model_tester, "text_model_tester"):
            return self._text_features_get_expected_num_attentions(
                model_tester.text_model_tester
            )
        ...
        raise ValueError(
            "Cannot determine the expected number of layers for text features"
        )
    ...
    def _image_features_get_expected_num_attentions(self, model_tester=None):
        ...
        if hasattr(model_tester, "vision_model_tester"):
            return self._image_features_get_expected_num_attentions(
                model_tester.vision_model_tester
            )
        elif (
        ...
        if hasattr(model_tester, "expected_num_hidden_layers"):
            ...
            return model_tester.num_hidden_layers
        raise ValueError(
            "Cannot determine the expected number of layers for image features"
        )
    ...
    def _audio_features_get_expected_num_attentions(self, model_tester=None):
        ...
        if hasattr(model_tester, "audio_model_tester"):
            return self._audio_features_get_expected_num_attentions(
                model_tester.audio_model_tester
            )
        elif (
        ...
        if hasattr(model_tester, "expected_num_hidden_layers"):
            ...
            return model_tester.num_hidden_layers
        raise ValueError(
            "Cannot determine the expected number of layers for audio features"
        )
    ...
    def _video_features_get_expected_num_attentions(self, model_tester=None):
        ...
        if hasattr(model_tester, "video_model_tester"):
            return self._video_features_get_expected_num_attentions(
                model_tester.video_model_tester
            )
        if hasattr(model_tester, "vision_model_tester"):
            return self._video_features_get_expected_num_attentions(
                model_tester.vision_model_tester
        ...
        if hasattr(model_tester, "expected_num_hidden_layers"):
            ...
            return model_tester.num_hidden_layers
        raise ValueError(
            "Cannot determine the expected number of layers for video features"
        )
        ...
        for model_class in self.all_model_classes:
            ...
            if return_dict in (True, None):
                self.assertTrue(
                    ...
                    "get_text_features() must return a BaseModelOutput",
                )
                self.assertTrue(
                ...
                self.assertIsInstance(
                    outputs,
                    tuple,
                    "get_text_features() must return a tuple if return_dict=False",
                )
    ...
    def test_get_text_features_hidden_states(self):
        def check_hidden_states_output(inputs_dict, config, model_class):
            ...
            expected_num_hidden_states = (
                self._text_features_get_expected_num_hidden_states()
            )
            self.assertIsNotNone(hidden_states, "hidden_states should not be None")
            self.assertEqual(
                len(hidden_states),
                expected_num_hidden_states,
                "Number of hidden states layers mismatch",
            )
    ...
    def test_get_text_features_attentions(self):
        def check_attentions_output(inputs_dict, config, model_class):
            ...
            self.assertEqual(
                ...
                "Number of attention layers mismatch",
            )
    ...
    def test_get_image_features_output(self, return_dict: bool | None):
        for model_class in self.all_model_classes:
            ...
            if return_dict in (True, None):
                self.assertTrue(
                    isinstance(outputs, ModelOutput),
                    "get_image_features() must return a BaseModelOutput",
                )
                self.assertTrue(
                    hasattr(outputs, "last_hidden_state"),
                ...
                vision_config = (
                    vision_config.vq_config
                    if hasattr(vision_config, "vq_config")
                    else vision_config
                )
                vision_config = (
                    vision_config.model_args
                    if hasattr(vision_config, "model_args")
                    else vision_config
                )
                ...
                for attr in attribute_candidates:
                    ...
                    raise ValueError(
                        "Cannot find the hidden size attribute in vision_config"
                    )
                ...
                self.assertIsInstance(
                    outputs,
                    tuple,
                    "get_image_features() must return a tuple if return_dict=False",
                )
    ...
    def test_get_image_features_hidden_states(self):
        def check_hidden_states_output(inputs_dict, config, model_class):
            ...
            hidden_states = outputs.hidden_states
            expected_num_hidden_states = (
                self._image_features_get_expected_num_hidden_states()
            )
            self.assertIsNotNone(hidden_states, "hidden_states should not be None")
            self.assertEqual(
                len(hidden_states),
    ...
    def test_get_audio_features_output(self, return_dict: bool | None):
        for model_class in self.all_model_classes:
            ...
            if return_dict in (True, None):
                self.assertTrue(
                    isinstance(outputs, ModelOutput),
                    "get_audio_features() must return a BaseModelOutputWithPooling",
                )
                self.assertTrue(
                    hasattr(outputs, "last_hidden_state"),
                ...
                audio_config = (
                    config.audio_config if hasattr(config, "audio_config") else config
                )
                ...
                self.assertIsInstance(
                    outputs,
                    tuple,
                    "get_audio_features() must return a tuple if return_dict=False",
                )
    ...
    def test_get_audio_features_hidden_states(self):
        def check_hidden_states_output(inputs_dict, config, model_class):
            ...
            hidden_states = outputs.hidden_states
            expected_num_hidden_states = (
                self._audio_features_get_expected_num_hidden_states()
            )
            self.assertIsNotNone(hidden_states, "hidden_states should not be None")
            self.assertEqual(
                len(hidden_states),
    ...
    def test_get_video_features_output(self, return_dict: bool | None):
        for model_class in self.all_model_classes:
            ...
            if return_dict in (True, None):
                self.assertTrue(
                    isinstance(outputs, ModelOutput),
                    "get_video_features() must return a BaseModelOutput",
                )
                self.assertTrue(
                    hasattr(outputs, "last_hidden_state"),
                ...
                self.assertIsInstance(
                    outputs,
                    tuple,
                    "get_video_features() must return a tuple if return_dict=False",
                )
    ...
    def test_get_video_features_hidden_states(self):
        def check_hidden_states_output(inputs_dict, config, model_class):
            ...
            hidden_states = outputs.hidden_states
            expected_num_hidden_states = (
                self._video_features_get_expected_num_hidden_states()
            )
            self.assertIsNotNone(hidden_states, "hidden_states should not be None")
            self.assertEqual(
                len(hidden_states),
    ...
    def test_capture_outputs_decorator(self):
        ...
        def new_set(self, value):
            ...
            if any(v > 1 for v in COUNTER.values()):
                raise ValueError(
                    "You're calling `capture_outputs` several time in a chain!"
                )
...
def find_parent_traceback(
    full_param_name: str, model: PreTrainedModel
) -> tuple[str, str, str]:
